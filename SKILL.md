...
5. Write in the user's requested language; when the user writes in Chinese,
   use plain Chinese. Show:
...
- `忘记这个仓库/主题的偏好` → `forget-repo` or `forget-topic`; explain that
  historical feedback remains in the audit log.
- `重置全部偏好` → ask for confirmation immediately before recording
